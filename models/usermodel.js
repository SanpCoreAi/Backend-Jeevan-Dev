...
exports.findByDoctorId = async (doctor_id) => {
  const [rows] = await db.query(
    "SELECT * FROM users WHERE doctor_id = ?",
    [doctor_id]
  );
  return rows;
};
...
exports.markEmailVerified = async (id) => {
  await db.query(
    "UPDATE users SET email_verified = 1, verificationToken = NULL WHERE id = ?",
    [id]
  );
};

exports.findUsers = async (filters = {}, limit = 10, offset = 0) => {
  try {
    let sql = `
    ...
    Object.entries(filters).forEach(([key, value]) => {
      ...
      if (["doctor_id", "role_id", "email_verified"].includes(key)) {
        sql += ` AND ${key} = ?`;
        params.push(value);
      }
    ...
    console.error("DB Error (findUsers):", error.message);
    throw error;
  }
};