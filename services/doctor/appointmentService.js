...
exports.bookAppointment = async (patientId, patientEmail, doctorId, body) => {
  ...
  if (!appointment_date || !start_time || !end_time || !hospital_name) {
  ...
  for (const s of schedules) {
    ...
    // ✅ hospital match
    if (
      String(s.hospital_name).toLowerCase().trim() !==
      hospital_name.toLowerCase().trim()
    ) {
      continue;
    }

    const apptDate = new Date(appointment_date).toISOString().split("T")[0];
    ...
    const candidate = await SlotModel.getActiveSlot(
      s.id,
      doctorId,
      apptDate,
      start24
  ...
  if (!schedule) {
    return { success: false, message: "Doctor not available for this hospital/date" };
  }
